...
---@enum pasta.kit.IO.UV.AccessMode
IO.AccessMode = {
  r = 'r',
  rs = 'rs',
  sr = 'sr',
...
---@enum pasta.kit.IO.WalkStatus
IO.WalkStatus = {
  SkipDir = 1,
  Break = 2,
}

---@type fun(path: string): pasta.kit.Async.AsyncTask
...
---@type fun(path: string, mode: integer): pasta.kit.Async.AsyncTask
IO.fs_mkdir = Async.promisify(uv.fs_mkdir)

---@type fun(from: string, to: string, option?: { excl?: boolean, ficlone?: boolean, ficlone_force?: boolean }): pasta.kit.Async.AsyncTask
IO.fs_copyfile = Async.promisify(uv.fs_copyfile)

---@type fun(path: string, flags: pasta.kit.IO.UV.AccessMode, mode: integer): pasta.kit.Async.AsyncTask
IO.fs_open = Async.promisify(uv.fs_open)

---@type fun(fd: userdata): pasta.kit.Async.AsyncTask
IO.fs_close = Async.promisify(uv.fs_close)

---@type fun(fd: userdata, chunk_size: integer, offset?: integer): pasta.kit.Async.AsyncTask
IO.fs_read = Async.promisify(uv.fs_read)

---@type fun(fd: userdata, content: string, offset?: integer): pasta.kit.Async.AsyncTask
IO.fs_write = Async.promisify(uv.fs_write)

---@type fun(fd: userdata, offset: integer): pasta.kit.Async.AsyncTask
IO.fs_ftruncate = Async.promisify(uv.fs_ftruncate)

---@type fun(path: string, chunk_size?: integer): pasta.kit.Async.AsyncTask
...
---@param start_path string
---@param callback fun(err: string|nil, entry: { path: string, type: string }): pasta.kit.IO.WalkStatus?
